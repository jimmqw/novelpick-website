...
def check_nav_header(content):
    """导航栏存在：有<nav>标签 或 class=header 元素，且有渐变背景"""
    has_nav = '<nav' in content
    has_header_elem = '<header' in content
    # morai/novelpick: nav有fixed定位+背景色; gradient存在于页面某处即可
    has_fixed_nav = re.search(r'\.nav\s*\{[^}]*position:\s*fixed', content) is not None
    has_gradient = 'gradient' in content
    return (has_nav or has_header_elem) and has_gradient
...
def check_reading_time(content):
    return bool(re.search(r'min read|\d+\s*min\b|read time|min\b.*\bread\b|\u5206\u949F|\u9605\u8BFB', content, re.I))
...
for site in SITES:
    ...
    for f in html_files:
        ...
        if not check_nav_header(content):
