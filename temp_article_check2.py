...
def get_article_text(content):
    """Extract text from article-body div or article tag"""
    # Try class="article-body"
    m = re.search(r'<div[^>]*\bclass=["\'][^"\']*\barticle-body\b[^"\']*["\'][^>]*>(.*?)</div>\s*(?=<footer|<div class="recommended)', content, re.DOTALL | re.IGNORECASE)
    if m:
        text = re.sub(r'<[^>]+>', '', m.group(1)).strip()
        return len(text), text[:100]
    
    # Try id="article-body"
    m = re.search(r'<div[^>]*\bid=["\']article-body["\'][^>]*>(.*?)</div>\s*(?=<footer|<div class="recommended)', content, re.DOTALL | re.IGNORECASE)
    if m:
        text = re.sub(r'<[^>]+>', '', m.group(1)).strip()
        return len(text), text[:100]
    
    # Fallback: <article>...</article>
...
for site, fnames in files.items():
    ...
    for fname in fnames:
        ...
        print(f"  [{status}] {fname}: body={r['body_len']} chars, div={r['div_balance']}, ai={r['ai_phrases']}, kr={r['keep_reading']}")
        if flags:
